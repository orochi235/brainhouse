...
describe('clusterKey', () => {
  it('keys tool_use by kind + tool name', () => {
    const k = clusterKey(
      ev({ kind: 'tool_use', payload: { tool_use_id: 't', name: 'NewTool', input: {} } }),
    );
    expect(k).toBe('tool_use|NewTool');
  });
  it('keys text events by kind + detected markers', () => {
    const k = clusterKey(
      ev({ kind: 'assistant_text', payload: { text: 'hi <new-marker>stuff</new-marker>' } }),
    );
    expect(k).toBe('assistant_text|<new-marker');
  });
  it('keys markerless text by kind alone', () => {
    expect(clusterKey(ev({ kind: 'assistant_text', payload: { text: 'plain' } }))).toBe(
      'assistant_text|',
    );
  });
  it('keys other kinds by kind', () => {
    expect(
      clusterKey(ev({ kind: 'system', payload: { subtype: null, content: null, level: null } })),
    ).toBe('system|');
  });
});
